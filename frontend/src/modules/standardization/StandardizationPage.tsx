...
export function StandardizationPage() {
  ...
  const products = useQuery({ queryKey: ['products-standardization'], queryFn: projectApi.products })
  const versions = useQuery({ queryKey: ['versions-standardization', productId], queryFn: () => projectApi.versions(productId!), enabled: Boolean(productId) })
  ...
  const assessment = useQuery({ queryKey: ['assessment', versionId], queryFn: () => standardizationApi.assess(versionId!), enabled })
  ...
  const flywheel = useQuery({ queryKey: ['standardization-flywheel', versionId], queryFn: () => standardizationApi.flywheel(versionId!), enabled })
  ...
  return <div className="standardization-page">
    ...
    {!versionId ? <Card><Empty description="请先选择产品版本" /></Card> : <Tabs className="standardization-tabs" items={[
      { key: 'baseline', label: <span><FileProtectOutlined />能力基线</span>, children: <BaselineView values={baselines.data ?? []} loading={baselines.isLoading} onEdit={setBaselineEditor} /> },
      { key: 'maturity', label: <span><BarChartOutlined />成熟度</span>, children: <MaturityView value={assessment.data} loading={assessment.isLoading} onRefresh={() => assessment.refetch()} /> },
      ...
      { key: 'debt', label: <span><AuditOutlined />标准化债务</span>, children: <DebtView versionId={versionId} values={debts.data ?? []} loading={debts.isLoading} /> },
      { key: 'cost', label: <span><DollarOutlined />成本归集</span>, children: <CostView versionId={versionId} value={costs.data} tasks={tasks.data ?? []} loading={costs.isLoading || tasks.isLoading} /> },
      { key: 'flywheel', label: <span><SyncOutlined />产品飞轮</span>, children: <FlywheelView value={flywheel.data} loading={flywheel.isLoading} /> },
    ]} />}
    <BaselineEditor versionId={versionId} value={baselineEditor} onClose={() => setBaselineEditor(undefined)} />
  </div>
}

function BaselineView({ values, loading, onEdit }: { values: Baseline[]; loading: boolean; onEdit(value: Baseline | null): void }) {
  ...
  return <div><div className="section-heading"><div><Typography.Title level={4}>产品能力卡</Typography.Title><span>{values.length} 张有效基线，同时说清标准范围、配置和扩展边界</span></div><Button type="primary" icon={<PlusOutlined />} onClick={() => onEdit(null)}>新建能力卡</Button></div>
...
function DebtView({ versionId, values, loading }: { versionId: number; values: StandardizationDebt[]; loading: boolean }) {
  ...
  return <Card loading={loading} title={<Space><AuditOutlined />标准化债务队列</Space>} extra={<Button type="primary" loading={evaluate.isPending} onClick={() => evaluate.mutate()}>评估高频二开</Button>}><Alert className="debt-alert" showIcon type="warning" message="同一扩展点出现在 5 个及以上项目时自动成为候选；关闭前必须完成版本验证。" />
    <Table rowKey="id" dataSource={values} pagination={false} columns={[{ title: '候选能力', render: (_, row) => <div><strong>{row.title}</strong><code className="cell-sub">{row.patternKey}</code></div> }, { title: '出现次数', dataIndex: 'occurrenceCount' }, { title: '覆盖项目', dataIndex: 'distinctProjects', render: value => `${value} 个` }, { title: '状态', dataIndex: 'status', render: value => <Tag color={value === 'CLOSED' ? 'success' : 'processing'}>{debtLabels[value]}</Tag> }, { title: '操作', render: (_, row) => row.status === 'CLOSED' ? <CheckCircleOutlined className="success-icon" /> : <Button size="small" onClick={() => next(row)}>{debtLabels[debtStates[debtStates.indexOf(row.status) + 1]]}<ArrowRightOutlined /></Button> }]} />
...
function CostView({ versionId, value, tasks, loading }: { versionId: number; value?: CostSummary; tasks: CustomDevTask[]; loading: boolean }) {
  ...
  return <div><Row gutter={12} className="cost-kpis"><Col span={6}><Card loading={loading}><Statistic title="预估人天" value={value?.estimatedPersonDays ?? 0} suffix="人天" /></Card></Col><Col span={6}><Card loading={loading}><Statistic title="实际人天" value={value?.actualPersonDays ?? 0} suffix="人天" /></Card></Col><Col span={6}><Card loading={loading}><Statistic title="实际成本" value={value?.actualCost ?? 0} prefix="¥" /></Card></Col><Col span={6}><Card loading={loading}><Statistic title="成本偏差" value={overrun} prefix="¥" valueStyle={{ color: overrun > 0 ? '#f54a45' : '#2ea869' }} /></Card></Col></Row>
    <Card title="二开任务与成本明细" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setEditing(null)}>新建二开任务</Button>}><Table rowKey="id" loading={loading} dataSource={tasks} pagination={false} columns={[
      ...
      { title: '操作', render: (_, item: CustomDevTask) => <Button size="small" onClick={() => setEditing(item)}>编辑</Button> },
    ]} /></Card>
...
function BaselineEditor({ versionId, value, onClose }: { versionId?: number; value: Baseline | null | undefined; onClose(): void }) {
  const [form] = Form.useForm()
  const client = useQueryClient()
