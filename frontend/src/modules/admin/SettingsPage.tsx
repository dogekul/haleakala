...
export function SettingsPage() {
  ...
  return <section>
    ...
    <Row gutter={14}><Col span={16}><Card className="admin-surface" title="基础设置" loading={query.isLoading}><Form form={form} layout="vertical" onFinish={save.mutate} requiredMark="optional">
      ...
      <Row gutter={16}><Col span={12}><Form.Item label="默认时区" name="timezone" rules={[{ required: true }]}><SearchSelect options={['Asia/Shanghai', 'Asia/Hong_Kong', 'Asia/Singapore', 'UTC', 'Europe/London', 'America/New_York'].map(value => ({ value, label: value }))} /></Form.Item></Col><Col span={12}><Form.Item label="支持邮箱" name="supportEmail" rules={[{ type: 'email' }]}><Input prefix={<MailOutlined />} placeholder="support@example.com" /></Form.Item></Col></Row>
