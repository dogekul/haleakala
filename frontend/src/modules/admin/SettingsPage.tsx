...
export function SettingsPage() {
  ...
  const save = useMutation({ mutationFn: adminApi.saveSettings, onSuccess: async value => { form.setFieldsValue(value); await client.invalidateQueries({ queryKey: ['admin-settings'] }); message.success('系统设置已保存') }, onError: (error: Error) => message.error(error.message) })
  return <section>
