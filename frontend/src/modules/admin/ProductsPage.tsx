...
export function ProductsPage() {
  ...
  return <section>
    ...
    <AdminQueryAlert errors={[products.error, users.error, versions.error]} onRetry={() => { void Promise.all([products.refetch(), users.refetch(), versions.refetch()]) }} />
    <Row gutter={14}>
...
function ProductEditor({ value, users, onClose }: { value: Product | null | undefined; users: { id: number; displayName: string }[]; onClose(): void }) {
  ...
  useEffect(() => { if (value !== undefined) { form.resetFields(); form.setFieldsValue(value ?? { status: 'ACTIVE' }) } }, [form, value])
  ...
  return <Modal title={value ? '编辑产品' : '新建产品'} open={value !== undefined} onCancel={onClose} okText="保存" cancelText="取消" confirmLoading={save.isPending} onOk={() => form.submit()} destroyOnHidden><Form form={form} layout="vertical" preserve={false} onFinish={save.mutate}>
...
function VersionEditor({ product, value, onClose }: { product: Product; value: ProductVersion | null | undefined; onClose(): void }) {
  const [form] = Form.useForm()
  const client = useQueryClient()
  useEffect(() => { if (value !== undefined) { form.resetFields(); form.setFieldsValue(value ? { ...value, releaseDate: value.releaseDate ? dayjs(value.releaseDate) : null } : { status: 'ACTIVE' }) } }, [form, value])
  ...
  return <Modal title={`${value ? '编辑' : '新建'}版本 · ${product.name}`} open={value !== undefined} onCancel={onClose} okText="保存" cancelText="取消" confirmLoading={save.isPending} onOk={() => form.submit()} destroyOnHidden><Form form={form} layout="vertical" preserve={false} onFinish={save.mutate}>
