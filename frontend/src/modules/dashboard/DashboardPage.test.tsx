...
it('快速创建使用可绑定产品版本且切换产品会清空已选版本', async () => {
  ...
  const product = within(drawer).getByRole('combobox', { name: '产品' })
  ...
  await user.type(product, '生效 产品')
  expect(screen.queryByRole('option', { name: 'NEXT · 另一产品' })).not.toBeInTheDocument()
  ...
  expect(within(drawer).queryByRole('checkbox', { name: '创建后执行项目初始化 Skill' })).not.toBeInTheDocument()
  expect(within(drawer).getByText(/自动初始化七阶段.*Outline 项目文档/)).toBeVisible()
  ...
  expect(requests.some(path => path.includes('/agent-jobs'))).toBe(false)
})
