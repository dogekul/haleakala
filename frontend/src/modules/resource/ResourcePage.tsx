...
function AssignmentEditor({ open, value, team, onClose }: { open: boolean; value?: Assignment; team: TeamMember[]; onClose(): void }) {
  ...
  return <Drawer width={560} title={value?'调整资源分配':'新建资源分配'} open={open} onClose={onClose} extra={<Button type="primary" loading={save.isPending} onClick={()=>form.submit()}>保存分配</Button>}><Alert className="drawer-alert" showIcon type="info" message="重叠排期可以保存；总投入超过 100% 后会立即进入冲突预警。" /><Form form={form} layout="vertical" onFinish={save.mutate}><Form.Item name="userId" label="人员" rules={[{required:true}]}><Select options={team.map(item=>({value:item.userId,label:`${item.displayName} · ${item.jobTitle}`}))} /></Form.Item><Form.Item name="projectId" label="项目" rules={[{required:true}]}><Select options={projects.data?.map(item=>({value:item.id,label:`${item.code} · ${item.name}`}))} /></Form.Item><Form.Item name="role" label="项目角色" rules={[{required:true}]}><Input /></Form.Item><Form.Item name="dates" label="分配日期" rules={[{required:true}]}><DatePicker.RangePicker style={{width:'100%'}} /></Form.Item><Form.Item name="allocationPercent" label="投入比例 (%)" rules={[{required:true}]}><InputNumber min={1} max={100} style={{width:'100%'}} /></Form.Item></Form></Drawer>
}
