...
export function ResourcePage() {
  ...
  const [assignmentEditor, setAssignmentEditor] = useState<{ value?: Assignment }>()
  ...
  return <div className="resource-page">
    <div className="workshop-heading"><div><span className="eyebrow dark">RESOURCE COMMAND CENTER</span><Typography.Title level={2}>资源中心</Typography.Title><Typography.Paragraph>用结构化技能和真实投入比例排产，在冲突影响交付前完成调度。</Typography.Paragraph></div>
      <Space><DatePicker.RangePicker value={[dayjs(range[0]), dayjs(range[1])]} onChange={dates => dates?.[0] && dates[1] && setRange([dates[0].format('YYYY-MM-DD'), dates[1].format('YYYY-MM-DD')])} /><Button type="primary" size="large" icon={<PlusOutlined />} onClick={() => setAssignmentEditor({})}>新建分配</Button></Space></div>
    ...
    <Tabs className="resource-tabs" items={[
      ...
      { key: 'conflicts', label: <span>冲突预警{Boolean(conflicts.data?.length) && <b>{conflicts.data?.length}</b>}</span>, children: <ConflictView values={conflicts.data ?? []} onAdjust={value => setAssignmentEditor({ value })} /> },
    ]} />
    <ProfileEditor value={profile} onClose={() => setProfile(undefined)} />
    <AssignmentEditor open={Boolean(assignmentEditor)} value={assignmentEditor?.value} team={team.data ?? []} onClose={() => setAssignmentEditor(undefined)} />
  </div>
}
...
function ConflictView({ values, onAdjust }: { values: ResourceConflict[]; onAdjust(value: Assignment): void }) {
  return <div>{!values.length ? <Card><Empty description="当前时间段无排期冲突" /></Card> : <Row gutter={[12,12]}>{values.map(item => <Col span={12} key={`${item.userId}-${item.startDate}-${item.endDate}`}><Card className="conflict-card"><div className="conflict-head"><AlertOutlined /><div><strong>{item.displayName} 峰值负载达 {item.peakAllocationPercent}%</strong><span>{item.startDate} — {item.endDate} · {item.assignments.length} 项分配并发</span></div><Tag color="red">需调度</Tag></div><div className="conflict-projects">{item.assignments.map(assignment => <div key={assignment.id}><b>{assignment.projectCode}</b><span>{assignment.projectName}</span><strong>{assignment.allocationPercent}%</strong></div>)}</div><Button block onClick={() => onAdjust(item.assignments[0])}>调整资源分配</Button></Card></Col>)}</Row>}</div>
}
...
function ProfileEditor({ value, onClose }: { value?: TeamMember; onClose(): void }) {
  ...
  return <Drawer width={580} title={value ? `${value.displayName} · 能力档案` : ''} open={Boolean(value)} onClose={onClose}>{value && <><Typography.Title level={5}>基本档案</Typography.Title><Form form={profileForm} layout="vertical" onFinish={saveProfile.mutate}><Row gutter={12}><Col span={12}><Form.Item name="jobTitle" label="职位" rules={[{required:true}]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="location" label="地点"><Input prefix={<EnvironmentOutlined />} /></Form.Item></Col><Col span={12}><Form.Item name="weeklyCapacityHours" label="周产能"><InputNumber min={1} max={80} style={{width:'100%'}} /></Form.Item></Col><Col span={12}><Form.Item name="resourceStatus" label="状态"><Select options={['ACTIVE','LEAVE'].map(v=>({value:v,label:v}))} /></Form.Item></Col></Row><Button type="primary" htmlType="submit" loading={saveProfile.isPending}>保存档案</Button></Form><Typography.Title level={5} className="skill-editor-title">添加 / 更新技能</Typography.Title><Form form={skillForm} layout="vertical" initialValues={{proficiency:3,experienceMonths:12,certified:false}} onFinish={saveSkill.mutate}><Form.Item name="skillId" label="技能" rules={[{required:true}]}><SearchSelect options={catalog.data?.map((item:SkillCatalogItem)=>({value:item.id,label:`${item.name} · ${item.category}`}))} /></Form.Item><Row gutter={12}><Col span={12}><Form.Item name="proficiency" label="熟练度 1-5"><InputNumber min={1} max={5} style={{width:'100%'}} /></Form.Item></Col><Col span={12}><Form.Item name="experienceMonths" label="经验月数"><InputNumber min={0} style={{width:'100%'}} /></Form.Item></Col></Row><Form.Item name="certified" label="认证"><Select options={[{value:true,label:'已认证'},{value:false,label:'未认证'}]} /></Form.Item><Button htmlType="submit" loading={saveSkill.isPending}>保存技能</Button></Form></>}</Drawer>
}

function AssignmentEditor({ open, value, team, onClose }: { open: boolean; value?: Assignment; team: TeamMember[]; onClose(): void }) {
  const [form] = Form.useForm()
  const client = useQueryClient()
  const projects = useQuery({ queryKey: ['projects-resource'], queryFn: projectApi.list, enabled: open })
  useEffect(() => { if (!open) return; form.resetFields();form.setFieldsValue(value ? { userId:value.userId,projectId:value.projectId,role:value.role,dates:[dayjs(value.startDate),dayjs(value.endDate)],allocationPercent:value.allocationPercent } : { allocationPercent:100 }) },[form,open,value])
  const save = useMutation({ mutationFn: (values: Record<string,unknown> & { dates: [Dayjs,Dayjs] }) => { const {dates,...fields}=values;const input={...fields,startDate:dates[0].format('YYYY-MM-DD'),endDate:dates[1].format('YYYY-MM-DD'),...(value ? {status:value.status,version:value.version} : {})};return value ? resourceApi.updateAssignment(value.id,input) : resourceApi.assign(input) }, onSuccess: async () => { await Promise.all([client.invalidateQueries({queryKey:['resource-team']}),client.invalidateQueries({queryKey:['resource-assignments']}),client.invalidateQueries({queryKey:['resource-load']}),client.invalidateQueries({queryKey:['resource-conflicts']})]);form.resetFields();onClose();message.success(value?'资源分配已更新':'资源分配已创建') }, onError: error => message.error(error instanceof Error?error.message:'资源分配保存失败') })
  return <Drawer width={560} title={value?'调整资源分配':'新建资源分配'} open={open} onClose={onClose} extra={<Button type="primary" loading={save.isPending} onClick={()=>form.submit()}>保存分配</Button>}><Alert className="drawer-alert" showIcon type="info" message="重叠排期可以保存；总投入超过 100% 后会立即进入冲突预警。" /><Form form={form} layout="vertical" onFinish={save.mutate}><Form.Item name="userId" label="人员" rules={[{required:true}]}><SearchSelect options={team.map(item=>({value:item.userId,label:`${item.displayName} · ${item.jobTitle}`}))} /></Form.Item><Form.Item name="projectId" label="项目" rules={[{required:true}]}><SearchSelect options={projects.data?.map(item=>({value:item.id,label:`${item.code} · ${item.name}`}))} /></Form.Item><Form.Item name="role" label="项目角色" rules={[{required:true}]}><Input /></Form.Item><Form.Item name="dates" label="分配日期" rules={[{required:true}]}><DatePicker.RangePicker style={{width:'100%'}} /></Form.Item><Form.Item name="allocationPercent" label="投入比例 (%)" rules={[{required:true}]}><InputNumber min={1} max={100} style={{width:'100%'}} /></Form.Item></Form></Drawer>
}
