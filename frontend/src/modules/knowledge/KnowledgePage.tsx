import {
  BookOutlined, CodeOutlined, DownloadOutlined, FileTextOutlined, PlusOutlined, ReadOutlined,
  SearchOutlined, SendOutlined, TeamOutlined, UploadOutlined,
} from '@ant-design/icons'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import {
  Alert, Button, Card, Col, Drawer, Empty, Form, Input, InputNumber, Row, Select,
  Space, Statistic, Tabs, Tag, Typography, Upload, message,
} from 'antd'
...
function KnowledgeDetail({ value, onClose }: { value?: KnowledgeItem; onClose(): void }) {
  ...
  return <Drawer width={700} title="知识阅读" open={Boolean(value)} onClose={onClose}>{value && <article className="knowledge-detail"><Space><Tag color={meta?.color}>{meta?.icon} {meta?.label}</Tag><Tag>{value.status}</Tag></Space><Typography.Title level={2}>{value.title}</Typography.Title><Typography.Paragraph className="knowledge-summary">{value.summary}</Typography.Paragraph>
    ...
    {value.type === 'TRAINING' && <><Alert showIcon type="success" message={`${value.audience} · ${value.durationMinutes ?? 0} 分钟`} description={value.fileObjectId ? `附件：${value.fileOriginalName ?? `文件 #${value.fileObjectId}`} · v${value.fileVersion ?? 1}` : '暂无附件'} />
      {value.fileObjectId && <Button icon={<DownloadOutlined />} href={`/api/v1/files/${value.fileObjectId}/download`} target="_blank">下载附件</Button>}</>}
...
function KnowledgeEditor({ value, onClose }: { value: KnowledgeItem | null | undefined; onClose(): void }) {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue(value ?? { type: 'CASE', visibility: 'ORGANIZATION' })
    setAttachment(value?.fileObjectId ? { id: value.fileObjectId, originalName: value.fileOriginalName ?? `文件 #${value.fileObjectId}`, fileVersion: value.fileVersion ?? 1, sizeBytes: value.fileSizeBytes ?? 0 } : undefined)
  }, [form, value])
  ...
  return <Drawer width={650} title={value ? '编辑知识' : '创建知识'} open={value !== undefined} onClose={onClose} extra={<Button type="primary" loading={save.isPending} onClick={() => form.submit()}>保存草稿</Button>}><Form form={form} layout="vertical" onFinish={save.mutate}><Form.Item label="知识类型" name="type" rules={[{ required: true }]}><Select options={Object.entries(typeMeta).map(([key, meta]) => ({ value: key, label: meta.label }))} /></Form.Item><Form.Item label="标题" name="title" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="一句话摘要" name="summary" rules={[{ required: true }]}><Input.TextArea rows={2} /></Form.Item><Form.Item label="正文" name="content" rules={[{ required: true }]}><Input.TextArea rows={7} /></Form.Item>
    ...
    {selectedType === 'TRAINING' && <Row gutter={12}><Col span={14}><Form.Item label="培训对象" name="audience" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={10}><Form.Item label="时长（分钟）" name="durationMinutes"><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col><Col span={24}><Form.Item name="fileObjectId" hidden><Input /></Form.Item><TrainingAttachment attachment={attachment} onUploaded={file => { setAttachment(file); form.setFieldValue('fileObjectId', file.id) }} /></Col></Row>}
...
function TrainingAttachment({ attachment, onUploaded }: { attachment?: UploadedFile; onUploaded(file: UploadedFile): void }) {
  const upload = async (file: File) => {
    const stored = attachment ? await knowledgeApi.addFileVersion(attachment.id, file) : await knowledgeApi.upload(file)
    onUploaded(stored)
    message.success(attachment ? '附件新版本已上传' : '附件已上传')
  }
  return <Card size="small" title="培训附件" extra={attachment && <Button type="link" icon={<DownloadOutlined />} href={`/api/v1/files/${attachment.id}/download`} target="_blank">下载</Button>}>
    {attachment && <Alert showIcon type="success" message={attachment.originalName} description={`已上传 · v${attachment.fileVersion}${attachment.sizeBytes ? ` · ${Math.ceil(attachment.sizeBytes / 1024)} KB` : ''}`} />}
    <Upload accept=".pdf,.ppt,.pptx,.doc,.docx,.zip,.mp4" showUploadList={false} customRequest={({ file, onSuccess, onError }) => upload(file as File).then(value => onSuccess?.(value)).catch(onError)}>
      <Button icon={<UploadOutlined />}>{attachment ? '上传新版本' : '选择文件并上传'}</Button>
    </Upload>
  </Card>
}
