...
it('创建项目使用独立的可绑定产品版本查询并在切换产品时清空版本', async () => {
  ...
  let version = within(drawer).getByRole('combobox', { name: '标品版本' })
  ...
  await user.click(within(drawer).getByRole('combobox', { name: '产品' }))
  await user.click(await screen.findByText('生效产品'))
  version = within(drawer).getByRole('combobox', { name: '标品版本' })
  await user.click(version)
...
it('没有启用客户时提示先维护客户主数据', async () => {
  ...
  await waitFor(() => expect(client.getQueryData(['active-customers'])).toEqual([]))
  const customer = within(drawer).getByRole('combobox', { name: '客户' })
