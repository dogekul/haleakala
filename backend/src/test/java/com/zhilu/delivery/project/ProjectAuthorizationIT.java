...
class ProjectAuthorizationIT {
  ...
  @Test
  void projectCannotReferenceManagerFromAnotherOrganization() throws Exception {
  ...
  @Test
  void riskCannotReferenceOwnerFromAnotherOrganization() throws Exception {
    mvc.perform(post("/api/v1/projects/{id}/risks", projectId)
            .with(actor(620, 620, "DELIVERY_MANAGER", "project:write")).with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"title\":\"跨组织风险负责人\",\"category\":\"DELIVERY\","
                + "\"probability\":2,\"impact\":2,\"ownerUserId\":623}"))
        .andExpect(status().isNotFound());
  }

  @Test
  void milestoneCannotReferenceOwnerFromAnotherOrganization() throws Exception {
    mvc.perform(post("/api/v1/projects/{id}/milestones", projectId)
            .with(actor(620, 620, "DELIVERY_MANAGER", "project:write")).with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"name\":\"跨组织里程碑负责人\",\"dueDate\":\"2026-12-01\","
                + "\"ownerUserId\":623}"))
        .andExpect(status().isNotFound());
  }
