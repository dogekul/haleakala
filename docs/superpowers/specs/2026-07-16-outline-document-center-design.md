...
本次改造将 Outline 作为全系统唯一的文档正文、目录和修订历史存储中心：
...
本系统继续负责业务权限、项目关系、知识分类、模版配置、门禁状态和审计；Outline 负责 Markdown 正文、目录和修订版本。培训课件等二进制附件继续由现有 MinIO 文件中心保存，Outline Attachment 预签名上传与完整离线包导出留作后续独立能力。
...
flowchart LR
    ...
    API --> OUTLINE["Outline API<br/>Markdown、目录、修订历史"]
...
标题与可编辑正文以 Outline 为准。现有 `content_text` 在迁移期间作为只读回退字段；迁移全部完成并稳定运行后再单独评估移除，不在本次直接删除。

知识列表需要的标题和摘要保留为本地查询缓存，创建、保存、迁移及后台同步都会刷新缓存。缓存不能作为文档编辑或导出的正文来源。

代码片段和培训材料的专属元数据继续保存在 MySQL；培训附件继续使用现有文件中心。正文中的远程图片不由后端主动抓取，避免 SSRF。
...
- Outline 源文档 ID。
- 发布时的 Outline 修订版本。
- 发布时的标题和 Markdown 不可变快照。

只有启用且已发布的模版参与项目初始化。发布动作同时记录当前 Outline 修订、标题和 Markdown；这些字段只作为异步创建项目副本的不可变输入，不作为可编辑正文或导出来源。
...
- 项目 ID 和阶段编码。
- 来源模版 ID、来源模版发布版本和创建项目时固化的不可变标题/Markdown。
- Outline 文档映射。
- 必需或可选。
...
1. 创建项目、七阶段和项目成员。
2. 在同一事务中将当时启用且已发布的模版 ID、阶段、必需性、发布修订和不可变正文快照写入项目文档记录。
3. 创建一条项目文档初始化任务，状态为待处理。
4. 提交项目事务并立即返回成功。
5. 后台任务调用 Outline API：
   - 确保“项目文档”根目录存在。
   - 创建或找到项目索引文档。
   - 创建七个阶段索引文档。
   - 读取项目创建事务已经固化的模版快照。
   - 将快照 Markdown 复制为对应阶段的项目文档。
   - 保存所有 Outline ID 和来源版本。
6. 全部完成后将项目文档空间状态更新为正常。
...
- 每个项目、阶段和模版只生成一个有效映射。
- 重复执行不会创建重复目录或文档。
- 模版在项目创建后再次编辑或发布，不改变该项目的初始化输入。

Outline 不可用时：

- 项目仍创建成功。
- 初始化任务记录失败原因并按退避策略重试。
- 运行中的任务持有数据库租约并定期续租；只有租约过期的任务才能被其他实例回收，完成和失败写入必须匹配原租约令牌。
- V15 升级数据若缺少不可变快照，只能在 Outline 当前修订仍等于原发布修订时自动补齐；无法证明修订一致时必须阻止复制并给出重新发布模版或重建项目的明确操作，不得静默跳过或使用空字符串占位。
