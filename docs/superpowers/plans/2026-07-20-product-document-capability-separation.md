...
- 后端全量测试：339 个，0 failures/errors。
...
- MySQL 8 验证：V20 在全新数据库执行通过；显式事务覆盖全部持久化 DML，临时表期间故障探针确认未提交变更会回滚；迁移前校验 31 个精确子模块编码，提交前再校验全部模块、功能、文档与版本清单数量。
- 兼容验证：商机文档已改为读取显式关联的产品文档 Spec；文档节点类型创建后不可变更。
